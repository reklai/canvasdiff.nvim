...
local function controller_groups(prefix)
  ...
  for _, autocmd in ipairs(vim.api.nvim_get_autocmds({})) do
    ...
    if name
        and (name == prefix or name:sub(1, #prefix + 1) == prefix .. ".")
...
local function reset_auxiliary_owners()
  ...
  for _, prefix in ipairs({ "canvasdiff.syntax", "canvasdiff.status_column" }) do
...
T["lifecycle_ racing terminal paths dispose and persist exactly once"] = function()
  with_canvas(function(ctx)
    ...
    local specs = {
      ...
      { name = "hl.detach", target = require("canvasdiff.ui").syntax, method = "detach" },
...
T["lifecycle_ a queued old callback cannot dispose its replacement"] = function()
  with_canvas(function(ctx)
    ...
    with_spies({
      ...
      H.eq(counts["sidebar.close"], sidebar_closes_after_open + 1)
