...
T["session_ committed range API is read-only and restores on reopen"] = function()
  ...
  in_repo(root, {}, function(fm)
    ...
    assert(winbar:find("topic → HEAD", 1, true),
      "two-dot comparison uses the normalized directional label")
    ...
    assert(restored:find("topic → HEAD", 1, true),
      "reopen restores the same comparison label")
    fm.close()
  end)
...
T["session_ restored lens derives its live label from identity"] = function()
  ...
  in_repo(root, {}, function(fm)
    ...
    assert(winbar:find("main → WORKTREE", 1, true),
      "the label is derived from normalized identity, not persisted copy")
    fm.close()
  end)
