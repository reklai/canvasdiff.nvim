...
- `lua/finding_myself/canvas.lua` — MODIFY: add `insert_section` (new primitive; mirrors `replace_section`'s view-correction discipline).
- `lua/finding_myself/collect.lua` — NEW: worktree/HEAD content collection (`collect.files(root)`), extracted verbatim from init.lua's private helpers.
- `lua/finding_myself/watch.lua` — NEW: reconcile engine + triggers (`start`/`stop`/`reconcile`, module-level `on_empty` callback).
- `lua/finding_myself/hl.lua` — MODIFY: add `hl.detach(state)` (augroup + timer + live_state cleanup; approved follow-up from the Phase 2 final review).
- `lua/finding_myself/init.lua` — MODIFY: use collect.lua; start/stop watch on open/close; detach hl on close.
- `lua/finding_myself/config.lua` — MODIFY: `watch` defaults.
...
**Files:**
- Modify: `lua/finding_myself/canvas.lua`
...
Add to `lua/finding_myself/canvas.lua` (after `replace_section`, before `return M`):
...
```bash
git add lua/finding_myself/canvas.lua tests/test_canvas.lua
...
**Files:**
- Create: `lua/finding_myself/collect.lua`
- Create: `lua/finding_myself/watch.lua` (reconcile half only — triggers are Task 3)
- Modify: `lua/finding_myself/init.lua` (use collect.lua; drop the now-moved private helpers)
...
Expected: FAIL — `module 'finding_myself.collect' not found`.

- [ ] **Step 3: Implement `collect.lua`**

Create `lua/finding_myself/collect.lua` — move (verbatim, including comments) `find_loaded_buf`, `read_worktree_content`, and `collect_files` out of init.lua:
...
In `init.lua`: add `local collect = require("finding_myself.collect")`, delete the three moved private functions, and change both call sites (`M.open`, `M.refresh`) from `collect_files(...)` to `collect.files(...)`.

- [ ] **Step 4: Implement the reconcile half of `watch.lua`**

Create `lua/finding_myself/watch.lua`:
...
```bash
git add lua/finding_myself/collect.lua lua/finding_myself/watch.lua lua/finding_myself/init.lua tests/test_watch.lua
...
**Files:**
- Modify: `lua/finding_myself/watch.lua` (triggers + start/stop)
- Modify: `lua/finding_myself/hl.lua` (`hl.detach`)
- Modify: `lua/finding_myself/config.lua` (`watch` defaults)
- Modify: `lua/finding_myself/init.lua` (start/stop/detach wiring)
- Modify: `README.md`
- Test: `tests/test_watch.lua` (trigger cases appended)

**Interfaces:**
- Consumes: Task 2's `watch.reconcile`; hl.lua's module internals (augroup `finding_myself.hl`, module-level `timer` and `live_state`); `config.options.watch`.
- Produces:
  - `watch.start(state, opts)` — `opts = { debounce_ms = 200 }` (enabled is checked by the caller). Stops any previous watch first (idempotent). Installs: augroup `finding_myself.watch` with `BufWritePost` (only files under `state.root`) and `FocusGained`, both funneling into a single debounced dirty timer; fs_event watchers on the repo root (non-recursive), `<root>/.git` (filtering out `*.lock` churn), and the parent directories of currently-changed files; watcher set refreshed after each reconcile.
  - `watch.stop()` — stops the timer, closes all fs_event handles, deletes the augroup; safe to call repeatedly/when never started.
  - `hl.detach(state)` — clears augroup `finding_myself.hl`, stops the module debounce timer, and resets `live_state` when it is `state`; nil-safe.
...
`lua/finding_myself/hl.lua` — add after `M.attach`:
...
`lua/finding_myself/init.lua`:
- add `local watch = require("finding_myself.watch")` to the requires;
...
```bash
git add lua/finding_myself/ tests/test_watch.lua README.md
