...
- Neovim ≥0.10; `vim.text.diff`/`vim.diff` shim lives ONLY in `lua/finding_myself/differ.lua` (`differ.hunks(old, new)` → list of `{old_start, old_count, new_start, new_count}`, `result_type="indices"`).
...
- Highlight priorities (fixed contract): line tier **100** (existing `finding_myself.canvas.hl` namespace), word-diff tier **105**, treesitter tier **110**. Word+TS marks live in namespace `finding_myself.canvas.ts`.
...
- `lua/finding_myself/hl.lua` — NEW: lang detection, parse cache (LRU 20), `section_ts_marks`, apply/evict engine (`attach`/`apply_now`/`invalidate`).
- `lua/finding_myself/worddiff.lua` — NEW: pure char-level word-diff marks per section.
- `lua/finding_myself/model.lua` — MODIFY: sections carry `old_text`/`new_text`.
- `lua/finding_myself/canvas.lua` — MODIFY: two optional hooks (`state.hooks.on_render_all`, `state.hooks.on_section_replaced(path)`).
- `lua/finding_myself/config.lua` — MODIFY: `highlight = { enabled, margin, debounce_ms }` defaults.
- `lua/finding_myself/init.lua` — MODIFY: attach on open, re-apply on refresh.
- `lua/finding_myself/jump.lua` — MODIFY: re-apply after back-splice.
...
**Files:**
- Modify: `lua/finding_myself/model.lua` (build_section return table)
- Create: `lua/finding_myself/hl.lua`
...
Expected: FAIL — `module 'finding_myself.hl' not found`. Also run `make test FILTER=model_section` → FAIL on missing `old_text`.

- [ ] **Step 3: Implement**

In `lua/finding_myself/model.lua`, extend the return table of `build_section`:
...
```bash
git add lua/finding_myself/hl.lua lua/finding_myself/model.lua tests/test_hl.lua tests/test_model.lua
...
**Files:**
- Create: `lua/finding_myself/worddiff.lua`
...
- Produces: `worddiff.section_marks(section) -> marks` — same mark shape as Task 1 but `priority = 105` and `group = "FmWordDel"|"FmWordAdd"`. Pairing rule: within each consecutive del-run followed by add-run, pair del k with add k; unpaired lines get no marks. Pairs where either side is empty, identical, or > 500 bytes are skipped. Cols are byte cols (UTF-8 safe) + 1 prefix.
...
Expected: FAIL — `module 'finding_myself.worddiff' not found`.
...
```bash
git add lua/finding_myself/worddiff.lua tests/test_worddiff.lua
...
**Files:**
- Modify: `lua/finding_myself/hl.lua` (append engine)
- Modify: `lua/finding_myself/canvas.lua` (hooks in `render_all` + `replace_section`)
- Modify: `lua/finding_myself/config.lua` (highlight defaults)
- Modify: `lua/finding_myself/init.lua` (attach/apply wiring)
- Modify: `lua/finding_myself/jump.lua` (apply after back)
- Modify: `README.md`
...
- Produces:
  - `hl.attach(state, opts)` — `opts = { margin = rows beyond viewport (default 100), debounce_ms = 30 }`; sets `state.ts = { ids_by_path = {}, margin, debounce_ms }`, installs `state.hooks`, a `WinScrolled` autocmd (augroup `finding_myself.hl`, cleared on re-attach), defines `FmWordAdd`/`FmWordDel` (→ `DiffText`, `default = true`), and runs one immediate `apply_now`.
...
Append to `tests/test_hl.lua` (before `return T`; add `local canvas = require("finding_myself.canvas")` at the top):
...
`lua/finding_myself/config.lua` — add to `M.defaults`:
...
`lua/finding_myself/init.lua` — add `local hl = require("finding_myself.hl")` to the requires; at the end of `M.open()` (after `set_canvas_keymaps(st)`):
...
`lua/finding_myself/jump.lua` — add `local hl = require("finding_myself.hl")` to the requires; in `M.back()`, after the final `winrestview` call:

```lua
  hl.apply_now(state)
```

`README.md` — in the features/config sections: note that diff content is syntax-highlighted with the user's own treesitter setup plus intra-line word-diff emphasis, and document the `highlight` config table (`enabled`, `margin` = rows beyond the viewport kept highlighted, `debounce_ms` = scroll debounce) and the `FmWordAdd`/`FmWordDel` groups (default: `DiffText`).
...
```bash
nvim --headless --clean -c "set rtp+=." -c "lua require('finding_myself').open()" \
  -c "lua local ns = vim.api.nvim_create_namespace('finding_myself.canvas.ts'); print('ts marks: ' .. #vim.api.nvim_buf_get_extmarks(0, ns, 0, -1, {}))" \
  -c "qa!"
```
...
```bash
git add lua/finding_myself/ tests/test_hl.lua README.md
