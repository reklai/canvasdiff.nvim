...
- Highlight groups: `CanvasDiffSidebarDir` → `Directory` (default=true), `CanvasDiffSidebarActive` → `Visual` (default=true); active entry marked via `line_hl_group` extmark in namespace `canvasdiff.sidebar`.
...
- `lua/canvasdiff/sidebar.lua` — NEW: pure tree building/rendering + window lifecycle/sync/select/cycle.
- `lua/canvasdiff/config.lua` — MODIFY: sidebar + cycle-key defaults.
- `lua/canvasdiff/init.lua` — MODIFY: open/close/refresh wiring + canvas cycle keymaps.
- `lua/canvasdiff/watch.lua` — MODIFY: `sidebar.refresh(state)` after reconcile mutations.
- `lua/canvasdiff/jump.lua` — MODIFY: `sidebar.refresh(state)` after back-splice.
...
**Files:**
- Create: `lua/canvasdiff/sidebar.lua` (pure half only)
...
Expected: FAIL — `module 'canvasdiff.sidebar' not found`.

- [ ] **Step 3: Implement the pure half**

Create `lua/canvasdiff/sidebar.lua`:
...
```bash
git add lua/canvasdiff/sidebar.lua tests/test_sidebar.lua
...
**Files:**
- Modify: `lua/canvasdiff/sidebar.lua` (window half)
...
Append to `tests/test_sidebar.lua` (add `local canvas = require("canvasdiff.canvas")` and `local model = require("canvasdiff.model")` at the top):
...
local function sidebar_buf()
  for _, b in ipairs(vim.api.nvim_list_bufs()) do
    if vim.api.nvim_buf_is_valid(b) and vim.api.nvim_buf_get_name(b):find("canvasdiff://sidebar") then
      return b
    end
  end
end
...
Append to `lua/canvasdiff/sidebar.lua` (before `return S`; add `local canvas = require("canvasdiff.canvas")` at the top):

```lua
local NS = vim.api.nvim_create_namespace("canvasdiff.sidebar")
local BUFNAME = "canvasdiff://sidebar"
...
local function ensure_hl_groups()
  vim.api.nvim_set_hl(0, "CanvasDiffSidebarDir", { link = "Directory", default = true })
  vim.api.nvim_set_hl(0, "CanvasDiffSidebarActive", { link = "Visual", default = true })
end
...
function S.close()
  if side then
    local win = side.win
    side = nil
    pcall(vim.api.nvim_del_augroup_by_name, "canvasdiff.sidebar")
...
function S.open(state, opts)
  ...
  local aug = vim.api.nvim_create_augroup("canvasdiff.sidebar", { clear = true })
...
**Files:**
- Modify: `lua/canvasdiff/sidebar.lua` (`S.cycle`)
...
`init.lua` — add `local sidebar = require("canvasdiff.sidebar")`; in `set_canvas_keymaps` add:
...
`watch.lua` — add `local sidebar = require("canvasdiff.sidebar")`; in `W.reconcile`, add `sidebar.refresh(state)` immediately after BOTH `hl.apply_now(state)` call sites (the 0↔N/guard path and the merge-walk tail).

`jump.lua` — add `local sidebar = require("canvasdiff.sidebar")`; in `M.back`, after the final `hl.apply_now(state)`: `sidebar.refresh(state)`.
...
```bash
git add lua/canvasdiff/ tests/test_sidebar.lua README.md
