...
**Goal:** MVP of `finding_myself` — a Neovim plugin rendering ALL changed files' diffs (working tree vs HEAD, alphabetical) in one scrollable scratch canvas; `<CR>` on any diff line jumps into the real file buffer (full LSP/treesitter); `<M-CR>` returns to the canvas at the exact semantic position with that file's diff regenerated.
...
- Neovim ≥0.10; use `vim.text.diff` when present, else `vim.diff` (shim lives ONLY in `lua/finding_myself/differ.lua`).
...
- Section anchors: extmarks in namespace `finding_myself/anchors` with `right_gravity=false, invalidate=false, undo_restore=false`; one per section start + one EOF sentinel. Never `nvim_buf_set_lines` across a foreign section's anchor row.
...
- All Lua modules under `lua/finding_myself/`; pure modules (`differ`, `model`, `render`, `viewport` resolution) must not touch `vim.api` window/buffer state except where stated.
...
**Files:**
- Create: `.gitignore`, `Makefile`, `README.md`, `tests/run.lua`, `tests/helpers.lua`, `tests/test_smoke.lua`, `lua/finding_myself/util.lua`
...
- Produces:
  ...
  - `lua/finding_myself/util.lua`: `U.list_slice(t, s, e)`, `U.clamp(n, lo, hi)`.
...
- [ ] **Step 4: Write `lua/finding_myself/util.lua` and `tests/test_smoke.lua`**
...
**Files:**
- Create: `lua/finding_myself/viewport.lua`, `tests/test_viewport.lua`
...
- [ ] **Step 3: Implement `lua/finding_myself/viewport.lua`** exactly per the match chain above. Keep it pure (no `vim.api`). ~60 lines.
...
**Files:**
- Create: `lua/finding_myself/git.lua`, `lua/finding_myself/differ.lua`, `tests/test_git.lua`, `tests/test_differ.lua`
...
**Files:**
- Create: `lua/finding_myself/model.lua`, `lua/finding_myself/render.lua`, `tests/test_model.lua`
...
- Produces (used by Tasks 7-9):
  ...
  - `render.section_hl(section) -> list of { row = 0based_offset, group = "FmFileHeader"|"FmHunkHeader"|"DiffAdd"|"DiffDelete" }` — one per non-ctx line; ctx lines get no mark.
  - Highlight groups: define `FmFileHeader` (default link `Title`) and `FmHunkHeader` (default link `Comment`) — definition itself happens in Task 7's canvas setup, NOT here (render stays pure; it only names groups).
...
**Files:**
- Create: `lua/finding_myself/canvas.lua`, `tests/test_canvas.lua`
...
- Produces (used by Tasks 8-9): a `Canvas` object (module-level singleton is fine for MVP):
  - `canvas.open(sections, opts) -> canvas_state` — creates/reuses the scratch buffer (name `finding-myself://canvas`, options per Global Constraints; define hl groups `FmFileHeader`→`Title`, `FmHunkHeader`→`Comment` with `default = true`), shows it in the current window (plain `nvim_win_set_buf` — the canvas takes over the current window; store `state.win`), renders all sections, places anchors.
...
**Files:**
- Create: `lua/finding_myself/jump.lua`, `tests/test_jump.lua`
...
- [ ] **Step 2: Run, FAIL. Implement `lua/finding_myself/jump.lua`** (~120 lines) per interface. Detail: to compute `status` on back, reuse the section's stored `status`; content read must handle `nvim_buf_get_lines` needing `table.concat(lines, "\n") .. "\n"` (empty buffer ⇒ `""`).
...
- Produces (public API):
  ...
  - `plugin/finding_myself.lua`: `:FindingMyself {open|close|toggle|refresh}` with completion, default subcommand `toggle`. Guard `vim.g.loaded_finding_myself`.
...
- [ ] **Step 2: Run, FAIL. Implement `config.lua`, `init.lua`, `plugin/finding_myself.lua`** per interface.

- [ ] **Step 3: Run FULL suite `make test` — all green.**

- [ ] **Step 4: Update README.md** — install (lazy.nvim spec), `:FindingMyself`, default keymaps table, MVP scope + roadmap phases (treesitter tier, live watch, sidebar, scrollbar, virtualization, persistence).
...
3. Manual smoke (documented in README, executed by a human later): in any dirty repo `nvim`, `:FindingMyself` → scroll, `<CR>` into a hunk (LSP attaches — it's a real `:edit`), edit, `<M-CR>` back → position and updated diff verified.
