...
- Collapsed placeholder line: `"▸ " .. path .. ("  (%d hunks, +%d −%d)"):format(nhunks, adds, dels)` (U+2212), highlighted `CanvasDiffFileHeader`, exactly 1 row.
...
- Base semantics: `"HEAD"` = worktree vs HEAD (current behavior); `"index"` = worktree vs index (`git show :0:path`) — unstaged-only review. `state.base` threads through collect/jump/watch; `:CanvasDiff base` toggles + refreshes + notifies.
- Session file: `stdpath("state") .. "/canvasdiff/" .. vim.fn.sha256(root) .. ".json"`; contents `{version=2, base, collapsed=[paths...], folds=[dirpaths...], view={path,new_lnum,content,screen_offset}, cursor={path,new_lnum,content}}`; written on canvas close + `VimLeavePre`; restore skips silently on any resolution failure or version mismatch.
...
- `lua/canvasdiff/render.lua` — MODIFY: `placeholder(section)`.
- `lua/canvasdiff/canvas.lua` — MODIFY: collapse-aware line/hl building, `set_collapsed`, `state.collapsed`.
- `lua/canvasdiff/scrollbar.lua` — MODIFY: `line_kinds(sections, collapsed)`.
- `lua/canvasdiff/hl.lua` — MODIFY: skip collapsed sections in `apply_now`.
- `lua/canvasdiff/virt.lua` — NEW: Tier-1 auto-virtualization policy.
- `lua/canvasdiff/motions.lua` — NEW: `]f [f ]h [h`.
- `lua/canvasdiff/statuscol.lua` — NEW: statuscolumn text + attach/detach autocmds.
- `lua/canvasdiff/git.lua` — MODIFY: `show(root, rev, path)` generalization.
- `lua/canvasdiff/collect.lua` — MODIFY: `files(root, base)`.
- `lua/canvasdiff/session.lua` — NEW: save/restore.
- `lua/canvasdiff/sidebar.lua` — MODIFY: `get_folds()`/`set_folds(folds, state)` accessors.
- `lua/canvasdiff/{config,init,jump,watch}.lua`, `plugin/canvasdiff.lua`, `README.md` — MODIFY (wiring).
...
- Produces:
  ...
  - `canvas.open` initializes `state.collapsed = {}`; `render_all`/`replace_section`/`insert_section` render a section as `{ render.placeholder(sec) }` with a single `CanvasDiffFileHeader` hl mark when `state.collapsed[sec.path]`, else as before. (Introduce two small locals in canvas.lua — `section_lines_for(state, sec)` and a collapsed flag threaded into `apply_section_hl(buf, start_row, sec, collapsed)` — and use them at ALL THREE render sites.)
...
**Files:** Create `lua/canvasdiff/virt.lua`. Modify `init.lua`, `watch.lua`, `config.lua`. Test: `tests/test_virt.lua`.
...
- Produces:
  ...
  - `virt.attach(state, opts)` — installs a debounced (50ms) WinScrolled trigger (augroup `canvasdiff.virt`, singleton discipline: callback resolves module `live` state, timer stopped on re-attach) + runs one immediate `apply`. `virt.detach()` clears augroup/timer/auto-set.
...
**Files:** Create `lua/canvasdiff/motions.lua`, `lua/canvasdiff/statuscol.lua`. Modify `init.lua`, `config.lua`. Test: `tests/test_motions_statuscol.lua`.
...
- `statuscol.text()` — for `%!v:lua.require'canvasdiff.statuscol'.text()`: pcall-guarded; `""` unless the current buffer is the canvas and a state is attached; per `vim.v.lnum`: entry `new_lnum` ⇒ `"%4d "`, del ⇒ `"   · "`, headers/collapsed/missing ⇒ 5 spaces. `statuscol.attach(state)` — stores the state, sets window-local `statuscolumn` on `state.win`, installs BufWinEnter (set on entering window, update stored win) / BufWinLeave (scheduled clear from `state.win` when the canvas left it) autocmds in augroup `canvasdiff.statuscol` (singleton discipline). `statuscol.detach()` — clears option (liveness-guarded), augroup, state.
...
**Files:** Modify `git.lua`, `collect.lua`, `init.lua`, `jump.lua`, `watch.lua`, `config.lua`, `plugin/canvasdiff.lua`. Test: `tests/test_base.lua`.
...
- init: `state.base = config.options.base` at open; both `collect.files` call sites pass `state.base`/`st.base`; `M.toggle_base()` — no state ⇒ notify+return; flips `state.base`, calls `M.refresh()`, notifies `"CanvasDiff: diff base = worktree vs " .. (base == "index" and "index (unstaged)" or "HEAD")`.
...
- plugin command: add `"base"` to SUBCOMMANDS → `require("canvasdiff").toggle_base()`. config: `base = "HEAD"`.
...
**Files:** Create `lua/canvasdiff/session.lua`. Modify `sidebar.lua` (fold accessors), `init.lua`, `config.lua` (`session = { enabled = true }`). Test: `tests/test_session.lua`.
...
- `session.path_for(root)` — `stdpath("state") .. "/canvasdiff/" .. vim.fn.sha256(root) .. ".json"`.
...
- [ ] **Step 6 (phase wrap):** README — document collapse (`<Tab>`/`za`, `<CR>` expands), virtualization thresholds, motions, statuscolumn, `:CanvasDiff base`, session persistence (+ `session = { enabled = false }` opt-out). Commit `docs: phase 6 README`.
